...
describe("custom feature contracts", function()
  ...
  it("keeps the custom colorscheme registry", function()
    ...
    for _, name in ipairs({ "tokyonight", "everforest", "catppuccin" }) do
  ...
  it("keeps Markdown, Jupyter, Python, and TeX contracts", function()
    ...
    contains("lua/pithyvim/plugins/extras/lang/jupyter.lua", { 'filetype == "ipynb"', '%.ipynb$', 'event = { { event = "BufReadCmd", pattern = "*.ipynb" } }', 'ft = { "ipynb" }', "anti_conceal = { enabled = false }" })
