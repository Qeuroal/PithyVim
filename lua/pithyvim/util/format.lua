...
function M.formatexpr()
  if PithyVim.has("conform.nvim") then
...
function M.info(buf)
  ...
  PithyVim[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    PithyVim.warn("No formatter available", { title = "PithyVim" })
  end
end
...
function M.health()
  ...
  local has_extra = vim.tbl_contains(Config.spec.modules, "pithyvim.plugins.extras.lsp.none-ls")
  if has_plugin and not has_extra then
    PithyVim.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in PithyVim.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `pithyvim.plugins.extras.lsp.none-ls` extra,",
...
function M.snacks_toggle(buf)
  return Snacks.toggle({
    ...
    get = function()
      ...
      return PithyVim.format.enabled()
    end,
    set = function(state)
      PithyVim.format.enable(state, buf)
    end,
  })
end
