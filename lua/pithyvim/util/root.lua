...
function M.detectors.lsp(buf)
  ...
  local clients = PithyVim.lsp.get_clients({ bufnr = buf })
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  PithyVim.info(lines, { title = "PithyVim Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("LazyRoot", function()
    PithyVim.root.info()
  end, { desc = "PithyVim roots for the current buffer" })
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("pithyvim_root_cache", { clear = true }),
...
function M.get(opts)
  ...
  return PithyVim.is_win() and ret:gsub("/", "\\") or ret
end
