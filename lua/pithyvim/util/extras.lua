...
---@class pithyvim.util.extras
local M = {}
M.buf = 0

---@type LazyExtraSource[]
M.sources = {
  { name = "PithyVim", desc = "PithyVim extras", module = "pithyvim.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("pithyvim.extras")
---@type string[]
M.state = nil
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #PithyVim.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function M.get()
  ...
  for _, source in ipairs(M.sources) do
    local root = PithyVim.find_root(source.module)
    if root then
      PithyVim.walk(root, function(path, name, type)
...
function M.get_extra(source, modname)
  PithyVim.plugin.handle_defaults = false
  ...
  if def and def.enabled == false and vim.startswith(modname, "pithyvim.plugins.extras.lang.") then
    recommended = false
  end
  ...
  return {
    ...
    managed = vim.tbl_contains(PithyVim.config.json.data.extras, modname) or not enabled,
    plugins = plugins,
    optional = optional,
  }
end
...
function X.new()
  ...
  self.float = Float.new({ title = "PithyVim Extras" })
...
function X:toggle()
  ...
  for _, extra in ipairs(self.extras) do
    if extra.row == pos[1] then
      ...
      PithyVim.config.json.data.extras = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, PithyVim.config.json.data.extras)
      M.state = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, M.state)
      if extra.enabled then
        table.insert(PithyVim.config.json.data.extras, extra.module)
        M.state[#M.state + 1] = extra.module
      end
      table.sort(PithyVim.config.json.data.extras)
      PithyVim.json.save()
      PithyVim.info(
        "`"
          .. extra.name
          ...
          .. "\nPlease restart PithyVim to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("PithyVim Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled PithyVim extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  ...
  if not extra.managed then
    ...
    elseif vim.tbl_contains(PithyVim.plugin.core_imports, extra.module) or origin == "default" then
      self:diagnostic({
  ...
  if extra.source.name ~= "PithyVim" then
  ...
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^pithyvim.plugins.extras.", "")
