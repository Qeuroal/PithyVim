---@class pithyvim.util.treesitter
local M = {}
...
---@return boolean ok, pithyvim.util.treesitter.Health health
function M.check()
  ...
  ---@class pithyvim.util.treesitter.Health: table<string,boolean>
  local ret = {
...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      PithyVim.error(lines, { title = "PithyVim Treesitter" })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  M._ts_cli_callbacks = { cb }

  ---@param ok boolean
  ---@param err? string
  local function done(ok, err)
    local callbacks = M._ts_cli_callbacks or {}
    M._ts_cli_callbacks = nil
