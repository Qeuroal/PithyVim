---@class pithyvim.util.cmp
local M = {}

---@alias pithyvim.util.cmp.Action fun():boolean?
---@type table<string, pithyvim.util.cmp.Action>
M.actions = {
  -- Native Snippets
...
function M.auto_brackets(entry)
  ...
  if vim.tbl_contains({ Kind.Function, Kind.Method }, item.kind) then
    ...
    --{{{> Qeuroal
    local next_char = vim.api.nvim_get_current_line():sub(cursor[2] + 1, cursor[2] + 1)
...
function M.setup(opts)
  ...
  require("cmp.utils.snippet").parse = function(input)
    ...
    return PithyVim.cmp.snippet_preview(input)
  end
  ...
  cmp.event:on("confirm_done", function(event)
    if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then
      PithyVim.cmp.auto_brackets(event.entry)
    end
  end)
  cmp.event:on("menu_opened", function(event)
    PithyVim.cmp.add_missing_snippet_docs(event.window)
  end)
end
