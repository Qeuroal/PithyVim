if pithyvim_docs then
  ...
  vim.g.pithyvim_picker = "telescope"
end
...
local picker = {
  ...
  -- cwd will default to pithyvim.util.get_root
  -- for `files`, git_files or find_files will be chosen depending on .git
  ---@param builtin string
  ---@param opts? pithyvim.util.pick.Opts
...
if not PithyVim.pick.register(picker) then
  return {}
end
...
return {
  ...
  -- directory is a git repo.
  {
    ...
    dependencies = {
      {
        ...
        config = function(plugin)
          PithyVim.on_load("telescope.nvim", function()
            local ok, err = pcall(require("telescope").load_extension, "fzf")
            if not ok then
              local lib = plugin.dir .. "/build/libfzf." .. (PithyVim.is_win() and "dll" or "so")
              if not vim.uv.fs_stat(lib) then
                PithyVim.warn("`telescope-fzf-native.nvim` not built. Rebuilding...")
                require("lazy").build({ plugins = { plugin }, show = false }):wait(function()
                  PithyVim.info("Rebuilding `telescope-fzf-native.nvim` done.\nPlease restart Neovim.")
                end)
              else
                PithyVim.error("Failed to load `telescope-fzf-native.nvim`:\n" .. err)
              end
            end
          end)
        end,
      ...
      { "<leader>uC", PithyVim.pick("colorscheme", { enable_preview = true }), desc = "Colorscheme with Preview" },
      {
        ...
        function()
          require("telescope.builtin").lsp_document_symbols({
            symbols = PithyVim.config.get_kind_filter(),
          })
        end,
        desc = "Goto Symbol",
      },
    ...
    opts = function()
      ...
      local find_files_no_ignore = function()
        local action_state = require("telescope.actions.state")
        local line = action_state.get_current_line()
        PithyVim.pick("find_files", { no_ignore = true, default_text = line })()
      end
      local find_files_with_hidden = function()
        local action_state = require("telescope.actions.state")
        local line = action_state.get_current_line()
        PithyVim.pick("find_files", { hidden = true, default_text = line })()
      end
  ...
  -- Flash Telescope config
  {
    ...
    opts = function(_, opts)
      if not PithyVim.has("flash.nvim") then
        return
      end
