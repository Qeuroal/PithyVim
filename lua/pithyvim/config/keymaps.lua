-- This file is automatically loaded by pithyvim.config.init

-- DO NOT USE `PithyVim.safe_keymap_set` IN YOUR OWN CONFIG!!
-- use `vim.keymap.set` instead
local map = PithyVim.safe_keymap_set

-- better up/down (I dont care)
map({ "n", "x" }, "j", "v:count == 0 ? 'gj' : 'j'", { desc = "Down", expr = true, silent = true })
map({ "n", "x" }, "<Down>", "v:count == 0 ? 'gj' : 'j'", { desc = "Down", expr = true, silent = true })
map({ "n", "x" }, "k", "v:count == 0 ? 'gk' : 'k'", { desc = "Up", expr = true, silent = true })
map({ "n", "x" }, "<Up>", "v:count == 0 ? 'gk' : 'k'", { desc = "Up", expr = true, silent = true })
-- map({ "n", "x" }, "gj", "'j'", { desc = "Down", expr = true, silent = true, noremap = false })
-- map({ "n", "x" }, "gk", "'k'", { desc = "Up", expr = true, silent = true, noremap = false })
...
map({ "i", "n", "s" }, "<esc>", function()
  vim.cmd("noh")
  PithyVim.cmp.actions.snippet_stop()
  return "<esc>"
...
-- map("n", "<leader>xq", "<cmd>copen<cr>", { desc = "Quickfix List" }) -- old
...
map({"n","t"}, "<c-/>",function() Snacks.terminal.focus(nil, { cwd = PithyVim.root() }) end, { desc = "Terminal (Root Dir)" })
map({"n","t"}, "<c-_>",function() Snacks.terminal.focus(nil, { cwd = PithyVim.root() }) end, { desc = "which_key_ignore" })
-- map({"n","t"}, "<c-/>",function() Snacks.terminal() end, { desc = "Terminal (cwd)" })
...
map("n", "<leader><tab><tab>", "<cmd>tab split<cr>", { desc = "New Tab with Split Buffer" })
