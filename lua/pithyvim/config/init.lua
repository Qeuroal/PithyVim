...
M.version = "1.16.0" -- x-release-please-version
PithyVim.config = M
...
local defaults = {
  ...
  colorscheme = function()
    ...
    if (type(schemeOpt) ~= "table") then
      ...
      return require("catppuccin").load()
    end
...
---@alias PithyVimDefault {name: string, group: string, extra: string, import: string, enabled?: boolean, origin?: "global" | "default" | "extra" }
---
local default_extras ---@type table<string, PithyVimDefault>

---@param name string
---@param extras PithyVimDefault[]
function M.register_defaults(name, extras)
  ...
  local ret ---@type PithyVimDefault?
  local use ---@type string?

  local global = vim.g["pithyvim_" .. name]
  if vim.tbl_contains(valid, global) then
    ...
    if global and global ~= "auto" then
      vim.notify(
        ("Invalid value for `vim.g.pithyvim_%s`: `%s`\nValid options are: %s"):format(
          name,
          global,
          table.concat(valid, ", ")
        ),
    ...
    for _, extra in ipairs(extras) do
      if PithyVim.has_extra(extra.extra) then
